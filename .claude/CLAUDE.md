...
Keep it short and accurate. Updated 2026-07-07 to reflect launchd pipeline and 16-league coverage. All 25 leagues now active as of 2026-07-26. Don't expand it into a full technical reference — that lives in `wiki/architecture.md`.
...
The pipeline is live. SofaScore match/player/team/shot data and Transfermarkt squad data are loaded into BigQuery (`real-zaragoza-500608`). Raw data lives in BQ — not in `wiki/`. Wiki pages may summarise or reference data findings but should never contain raw tables.

**Active leagues (25):** LaLiga2 (54), 1RFEF (17073), Serie B (53), Ligue 2 (182), Romanian SuperLiga (152), J1 League (196), Turkish Süper Lig (52), Norwegian Eliteserien (20), Austrian Bundesliga (45), Korean K League 1 (410), Brasileirao Serie B (390), Mozzart Bet Superliga (210), MLS (242), Allsvenskan (40), Eerste Divisie (131), Moldovan Super Liga (685), Eredivisie (37), Belgian Pro League (38), Liga Portugal (238), Bundesliga (35), 2. Bundesliga (44), Premier League (17), La Liga (8), Serie A (23), Ligue 1 (34). WC 2026 in separate dataset `wc_2026` (tournament_id=16, season_id=58210). Season IDs and full backfill queue in `.claude/agents/data-engineer/AGENT.md`.

**Processed layers:** Raw data in `raw`/`wc_2026`. ML models in `ml`. Processed data in three datasets: `bronze` (views, union + normalise), `silver` (tables, dedup + fixes), `gold` (tables, aggregated). dbt model definitions in `pipeline/dbt/models/{bronze,silver,gold}/`. Refreshed daily by Cloud Run Job `rz-dbt-refresh` (06:00 via Cloud Scheduler + launchd 11:00/20:00). Always query `silver` or `gold` — never `raw` directly.
...
**Capology wage pipeline:** `pipeline/cloud-run/scrapers/scraper_capology.py` scrapes reported gross wages for ~2500 players across the top 5 EU leagues (Premier League, La Liga, Bundesliga, Ligue 1, Serie A) from capology.com. Cloud Run Job `rz-capology-scraper` — quarterly (1 Jan/Apr/Jul/Oct). Writes to `raw.capology_wages`. Layered: `bronze.capology_wages` (view) → `silver.capology_wages` (deduped, loans excluded) → `gold.agg_player_wage_benchmarks` (P25/median/P75 by position+league, joined with TM market values). Use for scouting budget planning and wage-to-value ratio analysis.

**Transfermarkt multi-league pipeline:** `pipeline/cloud-run/scrapers/scraper_transfermarkt_leagues.py` scrapes all 25 active pipeline leagues including 1RFEF (Groups E3G1/E3G2). Quarterly — **must run locally** (GCP datacenter IPs blocked by Transfermarkt/Cloudflare). Cloud Run Job `rz-tm-scraper` exists but cannot be triggered from GCP; run via `pipeline/run_transfermarkt_leagues.sh` on a local machine. Next run: Oct 1, 2026. Writes to `raw.transfermarkt_players` (includes Zaragoza via LaLiga2). Layered: `bronze.tm_players` (view) → `silver.tm_players` (deduped by player+club+season) → gold tables (`agg_player_market_values`, `agg_scouting_player_season`). Zaragoza squad view (`silver.rz_squad`) is derived from `raw.transfermarkt_players WHERE LOWER(club_name) LIKE '%zaragoza%'`. TM position is source of truth for scouting (more granular than SofaScore). TM codes verified 2026-07-24.

**BQML wage prediction pipeline:** `pipeline/cloud-run/wage-predictor/predict_wages.py` trains `ml.wage_regression` (linear_reg: log-wage ~ log-market-value + position_group) on Capology × TM joined data, then predicts wages for all ~13k TM players. Per-league multipliers correct for top-5-EU-only training bias (see `wiki/wage-model.md`). Cloud Run Job `rz-wage-predictor` — quarterly (2 Jan/Apr/Jul/Oct, 1 day after TM). Writes to `raw.bqml_wage_predictions` (append-only — tracks wage evolution). Layered: `silver.bqml_wages` (dbt, latest per player) → `gold.agg_scouting_player_season` (COALESCE capology_actual, bqml_estimate). Coverage: 163 capology_actual + 11,694 bqml_estimate players.
