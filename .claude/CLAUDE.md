...
Active leagues: LaLiga2 (54), 1RFEF (17073), Serie B (53), Ligue 2 (182), Romanian SuperLiga (152), J1 League (196), Turkish Süper Lig (52), Norwegian Eliteserien (20), Austrian Bundesliga (45), Korean K League 1 (410), Brasileirao Serie B (390), Mozzart Bet Superliga (210), MLS (242), Allsvenskan (40), Eerste Divisie (131), Moldovan Super Liga (685). **In queue / backfilling (added 2026-07-22):** Eredivisie (37), Belgian Pro League (38), Liga Portugal (238), 2. Bundesliga (35) — IDs confirmed, data loading via launchd cadence. WC 2026 in separate dataset `wc_2026` (tournament_id=16, season_id=58210). Season IDs and full backfill queue in `.claude/agents/data-engineer/AGENT.md`.

**Processed layers:** Raw data in `raw`/`wc_2026`. Processed data in three datasets: `bronze` (views, union + normalise), `silver` (tables, dedup + fixes), `gold` (tables, aggregated). SQL definitions in `pipeline/sql/{bronze,silver,gold}/`. Always query `silver` or `gold` — never `raw` directly.
...
**Transfermarkt multi-league pipeline:** `pipeline/cloud-run/scrapers/scraper_transfermarkt_leagues.py` scrapes all active pipeline leagues (1RFEF excluded — too many teams). Run manually or via `pipeline/run_transfermarkt_leagues.sh`; suggested cadence is weekly. Writes to `rz_raw.transfermarkt_players` (separate from `rz_raw.transfermarkt_squad` which is Zaragoza-only). Processed layers: `rz_bronze.bronze_tm_players` → `rz_silver.silver_tm_players` → `rz_gold.gold_tm_players`. Gold table is the scouting join target for market value, age, nationality, contract expiry. TM codes verified 2026-07-24.
